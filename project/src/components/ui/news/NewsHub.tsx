...
import { Newspaper, CalendarClock, Shield, Activity, Search, ChevronDown, ChevronRight, ExternalLink } from 'lucide-react';
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!newsResult.error && newsResult.data) {
        const allNews: NotiziaIntelligence[] = newsResult.data.map((n: any) => ({
          id: n.id,
          titolo: n.titolo,
        ...
        const grouped: BoxNews[] = CATEGORIE_UTENTE.map(cat => ({
          categoria: cat,
          items: allNews.filter(n => n.classifica.categoria === cat).slice(0, MAX_ITEMS_PER_BOX),
        })).filter(box => box.items.length > 0);

        setNewsByCategory(grouped);
      }

      if (!scadenzeResult.error && scadenzeResult.data) {
        const allScadenze: ScadenzaIntelligence[] = scadenzeResult.data.map((s: any) => ({
          id: s.id,
          titolo: s.titolo,
          descrizione: s.descrizione || '',
          normativa: s.normativa || '',
          soggettiCoinvolti: s.soggetti_coinvolti || ['docenti'],
          dataScadenza: s.data_scadenza || '',
          priorita: s.priorita || 'media',
          impatto: s.impatto || 'nazionale',
          conseguenzeNonAzione: s.conseguenze_non_azione || '',
          link: s.link || '',
          tipo: s.tipo || '',
          guidaOperativa: s.guida_operativa || '',
          regione: s.regione || '',
        }));

        const grouped: BoxScadenza[] = CATEGORIE_SCADENZA.map(cat => ({
          categoria: cat,
          items: allScadenze.filter(s => s.tipo === cat).slice(0, MAX_ITEMS_PER_BOX),
        })).filter(box => box.items.length > 0);

        setScadenzeByCategory(grouped);
      }
    } catch {
        ...
        {!loading && activeTab === 'notizie' && (
          <div className="space-y-6">
            ...
            {newsByCategory.map(box => {
              ...
              return (
                <div key={box.categoria}
                  className={`rounded-3xl border overflow-hidden transition-all duration-300 ${
                    isExpanded ? 'border-brand-blu/30 shadow-medium' : 'border-slate-200/60 shadow-soft hover:shadow-md'
                  }`}>
                  {/* Box Header */}
                  <button onClick={() => setExpandedBox(isExpanded ? null : `news-${box.categoria}`)}
                    className="w-full flex items-center justify-between p-5 bg-gradient-to-r from-white to-gray-50/80 hover:from-gray-50/50 transition-colors">
                  ...
                  {/* Box Content — preview or expanded */}
                  {isExpanded ? (
                    <div className="p-5 pt-0 space-y-3 animate-fade-in-up">
                      ...
                      <Link to="/notizie-scadenze/archivio?tab=notizie"
                        className="flex items-center justify-center gap-2 py-2 text-brand-blu text-sm font-semibold hover:text-brand-blu/80 transition">
                    ...
                    <div className="px-5 pb-4 flex gap-3 overflow-x-auto">
                      {box.items.map(item => (
                        <div key={item.id}
                          className="flex-shrink-0 w-64 bg-white rounded-2xl border border-slate-200/60 p-3 hover:border-brand-blu/20 transition cursor-pointer"
                          onClick={() => setExpandedBox(`news-${box.categoria}`)}>
          ...
          <div className="space-y-6">
            {scadenzeByCategory.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm">Nessuna scadenza attiva al momento.</p>
              </div>
            )}
            {scadenzeByCategory.map(box => {
              ...
              return (
                <div key={box.categoria}
                  className={`rounded-3xl border overflow-hidden transition-all duration-300 ${
                    isExpanded ? 'border-brand-ambra/30 shadow-medium' : 'border-slate-200/60 shadow-soft hover:shadow-md'
                  }`}>
                  <button onClick={() => setExpandedBox(isExpanded ? null : `scad-${box.categoria}`)}
                    className="w-full flex items-center justify-between p-5 bg-gradient-to-r from-white to-amber-50/30 hover:from-amber-50/20 transition-colors">
                    <div className="flex items-center gap-3">
                      <CalendarClock size={22} className="text-brand-ambra" />
                      <div className="text-left">
                        <h3 className="text-lg font-bold text-[#0F172A]">{box.categoria}</h3>
                        <p className="text-xs text-gray-500">{box.items.length} {box.items.length === 1 ? 'scadenza' : 'scadenze'} attive</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${colorClass}`}>
                  ...
                  {isExpanded && (
                    <div className="p-5 pt-0 space-y-3 animate-fade-in-up">
                      {box.items.map(item => (
                        <ScadenzaCard key={item.id} item={item} />
                      ))}
                      <Link to="/notizie-scadenze/archivio?tab=scadenze"
                        className="flex items-center justify-center gap-2 py-2 text-brand-ambra text-sm font-semibold hover:text-brand-ambra/80 transition">
                        Vedi tutte <ChevronRight size={14} />
                      </Link>
                    </div>
                  )}
...
function NewsCard({ item }: { item: NotiziaIntelligence }) {
  const { criticita, categoria } = item.classifica;
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${critColors[criticita] || critColors.media}`}>
              {criticita.toUpperCase()}
            </span>
            <span className="text-[10px] text-gray-400 flex items-center gap-1">
              {formatDataItaliana(item.dataPubblicazione)}
            </span>
          </div>
        ...
        {item.link && (
          <a href={item.link} target="_blank" rel="noopener noreferrer"
            className="flex-shrink-0 p-1.5 text-gray-400 hover:text-brand-blu transition">
...
function ScadenzaCard({ item }: { item: ScadenzaIntelligence }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 transition ${
      isUrgente ? 'bg-red-50/50 border-red-200' : 'bg-white border-slate-200/60'
    }`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${prioritaColors[item.priorita] || prioritaColors.media}`}>
              {item.priorita?.toUpperCase() || 'MEDIA'}
            </span>
            {giorniRimanenti !== null && (
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                isUrgente ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
              }`}>
          ...
          <h4 className="text-sm font-bold text-[#0F172A] line-clamp-2">{item.titolo}</h4>
          {item.normativa && (
            <p className="text-[10px] text-gray-400 mt-1">Rif: {item.normativa}</p>
          )}
          {dataScadenza && (
            <p className="text-xs text-gray-500 mt-1">
              Scadenza: {dataScadenza.toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' })}
            </p>
          )}
        </div>
        {item.link && (
          <a href={item.link} target="_blank" rel="noopener noreferrer"
            className="flex-shrink-0 p-1.5 text-gray-400 hover:text-brand-ambra transition">
            <ExternalLink size={14} />
          </a>
        )}
      </div>
